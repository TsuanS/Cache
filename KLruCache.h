...
public:
    ...
    KHashLruCaches(size_t capacity, int sliceNum)
        : capacity_(capacity) // 初始化总容量
        , sliceNum_(sliceNum > 0 ? sliceNum : std::thread::hardware_concurrency()) // 初始化切片数量，若传入的切片数量小于0，则使用硬件并发数
    {
...
public:
    KHashLruKCache(int capacity, int historyCapacity, int k, int sliceNum)
        :capacity_(capacity), // 初始化总容量
        sliceNum_(sliceNum > 0 ? sliceNum : std::thread::hardware_concurrency()) // 默认使用硬件并发数
    {
        // 每个分片的大小
        ...
        for (int i = 0; i < sliceNum_; ++i)
        {
            ...
            lruSliceCaches_.emplace_back(
                new KLruKCache<Key, Value>(sliceSize, historyCapacity, k)
            );
        }
    }
...
private:
    size_t capacity_;
    int sliceNum_;  // 分片数量
    std::vector<std::unique_ptr<KLruKCache<Key, Value>>> lruSliceCaches_; // 分片缓存
};
